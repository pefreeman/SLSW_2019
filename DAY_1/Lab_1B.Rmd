...
10. Repeat Q16, but instead of displaying the output, save the output to a new variable called `df.new`.
```{r}
# FILL ME IN
```
...
17. Create a pairs plot for galaxies in the EDS and UDS fields only, showing `field`, `z.mode`, and `mass`. Again, call someone over if you are unsure about what to make of the output.
```{r}
```
